/**
 * Cliente REST do ThingSpeak (app mobile).
 *
 *  Leitura (uplink ESP → app):
 *    - getLatest():  último ponto do canal  → objeto `live`
 *    - getHistory(): série temporal         → array de pontos do gráfico
 *
 *  Escrita / controle (downlink app → ESP) via fila TalkBack:
 *    - sendCommand(): enfileira um comando que o ESP32 consome por polling
 *
 * O ThingSpeak não faz streaming, então o app faz polling do feed
 * (ver useSensorData) respeitando o intervalo mínimo de 15s do plano gratuito.
 */
 ...
 * `commands/execute.json` no próximo ciclo de polling. `position: 1` coloca
 * no topo da fila para a ação mais recente do usuário valer primeiro.
 */
export async function sendCommand(command) {
  if (!controlEnabled) {
    throw new Error("TalkBack não configurado (EXPO_PUBLIC_THINGSPEAK_TALKBACK_*)");
  }
